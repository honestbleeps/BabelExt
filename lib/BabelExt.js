...
var BabelExt = (function(Global, unsafeGlobal) {
  ...
  */
  ...
  /*
   * memoryStorage - abstracted functions for get/set/remove...
   */
  instance.memoryStorage = {
    get: function(key, callback) {
        var thisJSON =  {
          requestType: 'memoryStorage',
          operation: 'getItem',
          itemName: key
        };
        instance.bgMessage(thisJSON, callback);
  ...
  return { // public interface
    ...
    memoryStorage: {
      /*
       * memoryStorage.get - gets storage for [key], calls callback with [return value] as parameter
       */
      get: function(key, callback) {
        if (typeof(callback) !== 'function') {
          throw 'ERROR: no callback provided for BabelExt.memoryStorage.get()';
        }
        instance.memoryStorage.get(key, callback);
      },
      /*
       * memoryStorage.set - sets storage for [key] to [value], calls callback with key, value as parameters
       */
      set: function(key, value, callback) {
        instance.memoryStorage.set(key, value, callback || function() {});
      },
      /*
       * memoryStorage.remove - deletes storage item at [key], calls callback with key as parameter
       */
      remove: function(key, callback) {
        instance.memoryStorage.remove(key, callback || function() {});
      }
    },
    ...
    */
    XMLHttpRequest: function() {
      ...
      this.open = function(method, url, async, user, password) {
        ...
        if ( url == location.origin || url.search(location.origin+'/') == 0 || url.search(/^[a-z]+:/) == -1 ) {
          ...
          if ( url.search(BabelExt._xhr_regexp) == -1 ) {
            console.log( "Blocked request to non-whitelisted URL '" + url + "' - please add this to xhr_patterns in your extension settings" );
            throw "Please add '" + url + "' to xhr_patterns in your extension settings";
          }
